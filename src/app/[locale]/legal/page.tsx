...
export default async function LegalMentionsPage({
  ...
  return (
    <main className="bg-bg text-text mx-auto min-h-dvh w-full max-w-lg px-4 py-10">
      ...
      <h1 className="mt-2 text-[26px] font-semibold tracking-tight">{t('mentions')}</h1>
      <p className="text-text-secondary mt-4 text-[15px] leading-relaxed">{t('mentionsBody')}</p>
      <nav className="mt-8 flex flex-col gap-3 text-[14px]">
        <Link href="/legal/privacy" className="text-accent font-medium">
          {t('privacy')}
        </Link>
